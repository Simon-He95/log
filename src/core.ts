...
const config = useConfiguration<{ fileInfo: boolean, scoped: boolean }>('codeLoc.config')
...
export async function getLog() {
  ...
  for (const selection of Array.from(selections)) {
    ...
    if (!text) {
      ...
      return editor.edit((builder) => {
        builder.insert(createPosition(selection.end.line + 1, 0), append)
      })
    }
  ...
  editor.edit((builder) => {
    data.forEach(item => {
      ...
      logPrefix += text
      builder.insert(position, transformAppend(suffix, tab, logPrefix, text))
    })
  })
}
